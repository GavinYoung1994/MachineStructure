...
static int sum_vectorized(int n, int *a)
{
   __m128i sum = _mm_setzero_si128();
	for (int i = 0; i < n/4*4; i+=4)
	{
   		__m128i arr = _mm_loadu_si128((__m128i*)(a+i));
		sum = _mm_add_epi32(sum, arr);
	}
	int b[4] = {0,0,0,0};
	_mm_storeu_si128((__m128i*)b, sum);
	int rt = 0;
	for(int i = 0; i<4; i++){
		rt += b[i];
	}
...
static int sum_vectorized_unrolled(int n, int *a)
{
   __m128i sum = _mm_setzero_si128();
	...
	{
   		__m128i arr = _mm_loadu_si128((__m128i*)(a+i));
   		__m128i arr1 = _mm_loadu_si128((__m128i*)(a+i+4));
   		__m128i arr2 = _mm_loadu_si128((__m128i*)(a+i+8));
   		__m128i arr3 = _mm_loadu_si128((__m128i*)(a+i+12));
		sum = _mm_add_epi32(sum, arr);
		sum = _mm_add_epi32(sum, arr2);
		sum = _mm_add_epi32(sum, arr3);
		sum = _mm_add_epi32(sum, arr1);
	}
	int b[4] = {0,0,0,0};
	_mm_storeu_si128((__m128i*)b, sum);
	int rt = 0;
